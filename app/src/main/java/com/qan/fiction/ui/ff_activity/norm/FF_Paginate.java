...
public class FF_Paginate extends Paginate<Entry> {


    private final List<String> order = Arrays.asList("genreid1", "languageid", "sortid",
            "length", "characterid1", "characterid2", "statusid", "verseid1", "p", "timerange");
    ...
    /** When not using get in the fanfiction.net, the order of arguments matters. */
    ...
    public String url_mobile(String url, int page) {
        return url.replace("www.fanfiction.net", "m.fanfiction.net");
    }
    ...
    private void listDialog(final Entry e) {
        ...
        v.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position == 0) {
                    String url;
                    String siteId = getId(e.site, e.file);
                    if (dualPane)
                        url = WebUtils.format(normalView.get(FF_NET_S), siteId, "1");
                    else
                        url = WebUtils.format(mobileView.get(FF_NET_S), siteId, "1");
    ...
    private void showDialog(final Entry e) {
        ...
        builder.setPositiveButton(getString(R.string.read), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                Bundle b = new Bundle();
                b.putString("url", "https://m.fanfiction.net/s/" + e.getId());
                b.putInt("chapters", e.chapters);
                b.putInt("id", Integer.parseInt(e.getId()));
                b.putString("site", FF_NET_S);
                b.putBoolean("dual", dualPane);
                b.putString("title", e.title);
                getViewListener().openFragment(new FF_ReaderFragment(), b);
